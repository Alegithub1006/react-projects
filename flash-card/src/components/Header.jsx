...
export function Header() {
  ...
  const handleActiveLocalStorage = () => {
    isActive(!active);
  };
  ...
  return (
    <nav className="header">
      ...
      <button
        onClick={handleActiveLocalStorage}
        className={active ? "header-button" : "header-button-disabled"}>
        {active ? "active" : "disable"} local storage
      </button>
    </nav>
  );
}
